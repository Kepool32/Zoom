...
export const fetchMeetingRecords = async (domain: string, page: number, perPage: number,dateFrom?:Date | null,dateTo?:Date | null ,  searchName?:string,id?:number,entity?:string,name?:string) => {
    try {
        const response = await axios.post(`${API_BASE_URL}/records`, { domain, page, perPage,dateFrom,dateTo, searchName,id,entity,name});
